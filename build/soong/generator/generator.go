...
func init() {
	android.RegisterModuleType("wave_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = waveExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = waveExpandVariables(ctx, dep_file)
	...
	cmd := waveExpandVariables(ctx, String(g.properties.Cmd))
